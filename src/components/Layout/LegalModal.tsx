...
export const LegalModal: React.FC<LegalModalProps> = ({ isOpen, onClose, lang, initialTab = 'privacy' }) => {
  ...
  const content = {
    ar: {
      ...
      faq2A: "ميزة الحذف الحالية في النسخة التجريبية لا تضمن بعد إزالة البيانات من كل المجموعات؛ راجع التنفيذ قبل الاعتماد عليها كمحو كامل.",
      ...
      privacyText1: "هذه نسخة تجريبية نشطة. تبقى أسرار Gemini على الخادم، لكن تُحفظ رموز GitHub حالياً ضمن إعدادات المستخدم لدعم التدفق المصادق عليه الحالي؛ ولا يمثل ذلك خزنة رموز إنتاجية مخصصة.",
    ...
    en: {
      ...
      faq2A: "The current beta's delete action does not yet guarantee removal from every collection; review the implementation before relying on it for complete erasure.",
      ...
      privacyText1: "This is an active beta. Gemini secrets stay server-side, but GitHub integration tokens are currently stored in the user's settings for the existing authenticated client flow; this is not a dedicated production token vault.",
    ...
    de: {
      ...
      faq2A: "Die Löschfunktion der aktuellen Beta garantiert noch nicht die Entfernung aus jeder Sammlung; prüfen Sie die Implementierung vor einer vollständigen Löschung.",
      ...
      privacyText1: "Dies ist eine aktive Beta. Gemini-Schlüssel bleiben auf dem Server, GitHub-Integrationstoken werden für den aktuellen authentifizierten Client-Ablauf jedoch in den Benutzereinstellungen gespeichert; dies ist keine dedizierte Produktions-Token-Vault.",
